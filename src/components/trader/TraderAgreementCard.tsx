...
export default function TraderAgreementCard({ agreement, compact = false }: { agreement: CurrentUserAgreementSummary; compact?: boolean }) {
  ...
  return (
    <Link to={`/agreements/${encodeURIComponent(agreement.agreementId)}`} className="block rounded-xl px-1 transition-colors hover:bg-green-50/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-600">
    <article className={`grid gap-4 border-b border-ink/8 py-4 last:border-0 ${compact ? 'sm:grid-cols-[1fr_auto]' : 'md:grid-cols-[minmax(0,1.6fr)_minmax(130px,.7fr)_minmax(150px,.8fr)_auto]'}`}>
      <div className="flex min-w-0 items-start gap-3">
        <span className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-full bg-green-50 text-green-700"><Handshake size={18} /></span>
      ...
      <div className="flex items-center justify-between gap-4 md:justify-end">
        <p className="font-semibold tabular-nums text-ink">{amount ?? 'Amount unavailable'}</p>
        <span aria-hidden="true" className="text-green-700"><ArrowRight size={17} /></span>
      </div>
    </article>
    </Link>
  );
