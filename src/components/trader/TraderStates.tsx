...
function CompactState({
  ...
  return (
    <section className={`trader-compact-state trader-compact-state--${tone}`} role={role}>
      <span className="trader-compact-state-icon" aria-hidden="true">{icon}</span>
      <div className="min-w-0 flex-1">
        <p className="trader-compact-state-eyebrow">{eyebrow}</p>
        <h2>{title}</h2>
        {detail && <p>{detail}</p>}
      </div>
      {action}
...
export function TraderErrorState({ title, detail, onRetry }: StateProps) {
  return <CompactState
    ...
    tone="caution"
    eyebrow="Could not refresh"
    title={title}
    detail={detail}
    role="alert"
    action={onRetry ? <button type="button" onClick={onRetry} className="trader-compact-state-action"><RefreshCw size={14} /> Retry</button> : undefined}
  />;
}
