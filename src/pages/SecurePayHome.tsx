    ...
    {state === 'ready' && <div className="space-y-7">
      <section className="signed-in-market-hero" aria-labelledby="home-agreement-heading">
        <div className="relative z-[1] min-w-0">
          ...
          <button type="button" onClick={openAgreementPrompt} className="market-start-prompt" aria-label="Tell SecurePay what you want to agree">
            <span className="market-start-prompt-copy">
              <span className="market-start-prompt-label">What would you like to agree today?</span>
      ...
      <section className="market-section-shell flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center"><div className="flex gap-3"><span className="market-section-icon"><FileCheck2 size={19} /></span><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-green-700">Agreement first</p><h2 className="market-section-title mt-1">Your work stays primary.</h2><p className="mt-2 text-sm text-ink/55">Account and financial context only appears when SecurePay can prove it.</p></div></div><button type="button" onClick={openAgreementPrompt} className="hp-start-btn inline-flex min-h-11 items-center gap-2 px-5 text-sm">Start an agreement <ArrowRight size={15} /></button></section>
    </div>}
  </TraderShell>;
}
