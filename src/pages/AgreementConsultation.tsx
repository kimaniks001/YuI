...
/** MW-11 — real, paid, scoped Master consultation. The API owns every authority transition. */
export default function AgreementConsultation() {
  ...
  const currentIdentityId = user?.id || session?.user.id || null;
  ...
  const load = useCallback(async () => {
    ...
    if (detail.ok && detail.data) setActive(detail.data);
    if (scoped.ok && scoped.data) setContext(scoped.data); else setContext(null);
    if (chat.ok && chat.data) setMessages(chat.data); else setMessages([]);
    if (submittedOpinion.ok && submittedOpinion.data) setOpinion(submittedOpinion.data); else setOpinion(null);
    if (extensionList.ok && extensionList.data) setExtensions(extensionList.data); else setExtensions([]);
    setLoading(false);
  ...
  const isMaster = Boolean(active && currentIdentityId && active.masterIdentityId === currentIdentityId);
  const isInviter = Boolean(active && currentIdentityId && active.invitedByIdentityId === currentIdentityId);
  ...
  async function create(event: React.FormEvent) {
    ...
    const result = await createAgreementConsultation(session.accessToken, {
      agreementId,
      masterProfileId: selectedMasterId,
      requestedMinutes,
      briefingNotes: briefing.trim(),
    });
    setBusy(false);
    if (!result.ok || !result.data) { setNote(result.error || 'The Master could not be invited.'); return; }
    setSearchParams({ consultation: result.data.id }); setBriefing(''); setSelectedMasterId('');
  }
  ...
  async function lifecycle(action: 'accept' | 'decline' | 'close' | 'revoke') {
    ...
    setBusy(false);
    if (!result.ok) setNote(result.error || 'That action is not available.'); else void load();
  }
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    if (!result.ok) setNote(result.error || 'Message could not be sent.'); else { setMessage(''); void load(); }
  }
  ...
  async function submitOpinion(event: React.FormEvent) {
    ...
    if (!result.ok) setNote(result.error || 'Opinion could not be submitted.'); else { setOpinionText(''); void load(); }
  }

  async function requestExtension(event: React.FormEvent) {
    event.preventDefault(); if (!active) return;
    const additionalMinutes = Number(extensionMinutes);
    if (!Number.isInteger(additionalMinutes) || additionalMinutes < 1) return;
    ...
    if (!result.ok) setNote(result.error || 'Extension could not be requested.'); else void load();
  }
  ...
  async function decideExtension(requestId: string, approve: boolean) {
    ...
    setBusy(false); if (!result.ok) setNote(result.error || 'Extension decision could not be recorded.'); else void load();
  }
  ...
  return <TraderShell>
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-3">
      <Boundary icon={<ShieldCheck size={17} />} title="Advisory only" body="The opinion is immutable advice, not a ruling." />
      <Boundary icon={<Clock3 size={17} />} title="Rate is snapshotted" body="SecurePayAPI owns the fee, PaymentIntent and access clock." />
      <Boundary icon={<MessageSquare size={17} />} title="Scoped access" body="The Master sees only the consultation context the backend permits." />
    </div>
      ...
      <div className="space-y-5">
        <section className="rounded-2xl border border-ink/8 bg-white p-5 shadow-sm">
          ...
          <form onSubmit={create} className="mt-4 space-y-3">
            <select value={selectedMasterId} onChange={e => setSelectedMasterId(e.target.value)} required className="min-h-11 w-full rounded-xl border border-ink/15 bg-white px-3 text-sm"><option value="">Choose an available Master</option>{masters.map(m => <option key={m.id} value={m.id}>{m.category} · {m.ksNumber || 'Master'} · {formatMinorMoney(m.currency, m.rateMinorPerHour)}/hr</option>)}</select>
            {selectedMaster && <p className="text-xs text-ink/50">Advertised now: {formatMinorMoney(selectedMaster.currency, selectedMaster.rateMinorPerHour)}/hr. The consultation stores its own rate snapshot.</p>}
            <textarea value={briefing} onChange={e => setBriefing(e.target.value)} rows={3} required placeholder="What would you like the Master to look at?" className="w-full rounded-xl border border-ink/15 p-3 text-sm" />
            <label className="block text-xs font-semibold text-ink/55">Requested minutes<input value={minutes} onChange={e => setMinutes(e.target.value)} type="number" min="1" step="15" className="mt-1 min-h-11 w-full rounded-xl border border-ink/15 px-3 text-sm" /></label>
            <button disabled={busy} className="min-h-11 rounded-full bg-green-700 px-5 text-sm font-semibold text-white disabled:opacity-50">Invite Master</button>
          </form>
        </section>
        {consultations.length > 0 && <section className="rounded-2xl border border-ink/8 bg-white p-4 shadow-sm"><p className="px-1 text-xs font-bold uppercase tracking-[0.16em] text-green-700">This agreement</p><div className="mt-3 space-y-2">{consultations.map(c => <button key={c.id} onClick={() => setSearchParams({ consultation: c.id })} className={`w-full rounded-xl border p-3 text-left ${active?.id === c.id ? 'border-green-700/30 bg-green-50' : 'border-ink/8'}`}><strong className="block text-sm">Master consultation</strong><span className="text-xs text-ink/50">{c.status} · {formatMinorMoney(c.currency, c.rateMinorPerHourSnapshot)}/hr</span></button>)}</div></section>}
      </div>
