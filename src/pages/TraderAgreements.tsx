...
export default function TraderAgreements() {
  ...
  return <TraderShell>
    <TraderPageHeader eyebrow="Your agreements" title="Agreements" description="One list for every agreement you take part in, whatever your role in that agreement." />
    <div className="mb-5 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Agreement filters">
      {(['active', 'all', 'completed'] as const).map(item => <button key={item} type="button" role="tab" aria-selected={filter === item} onClick={() => setFilter(item)} className={`min-h-11 whitespace-nowrap rounded-full px-5 text-sm font-semibold capitalize ${filter === item ? 'bg-green-700 text-white' : 'border border-ink/10 bg-white text-ink/60'}`}>{item}</button>)}
    </div>
    ...
    {state === 'ready' && <section className="rounded-2xl border border-ink/8 bg-white p-5 shadow-sm sm:p-6">
      <div className="hidden grid-cols-[minmax(0,1.6fr)_minmax(130px,.7fr)_minmax(150px,.8fr)_auto] gap-4 border-b border-ink/8 pb-3 text-[11px] font-bold uppercase tracking-wider text-ink/35 md:grid"><span>Agreement</span><span>Your role</span><span>Status</span><span className="text-right">Amount</span></div>
      {visible.length ? visible.map(item => <TraderAgreementCard key={item.agreementId} agreement={item} />) : <TraderEmptyState title={filter === 'active' ? 'No active agreements' : 'Nothing here yet'} detail={filter === 'active' ? 'You have no active agreements right now. Create a SecureLink when you are ready to begin.' : 'Agreements matching this view will appear here.'} />}
    </section>}
    {state === 'ready' && totalElements > pageSize && <div className="mt-4 flex items-center justify-between text-sm">
      <button type="button" disabled={page <= 0} onClick={() => void goToPage(page - 1)} className="min-h-11 rounded-full border border-ink/10 px-4 disabled:opacity-40">Previous</button>
      <span className="text-ink/50">Page {page + 1} of {totalPages}</span>
      <button type="button" disabled={page + 1 >= totalPages} onClick={() => void goToPage(page + 1)} className="min-h-11 rounded-full border border-ink/10 px-4 disabled:opacity-40">Next</button>
    </div>}
    <p className="mt-4 text-xs leading-5 text-ink/40">Role, counterparty, and next-action fields are only shown when the backend provides them. SecurePay does not infer them from who created the agreement.</p>
  </TraderShell>;
}
