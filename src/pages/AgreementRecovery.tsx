...
export default function AgreementRecovery() {
  ...
  async function inviteMaster(event: React.FormEvent) {
    ...
    setBusy(true); setNotice(null);
    const result = await inviteRecoveryMaster(session.accessToken, room.recoveryCase.id, { masterProfileId: masterId, requestedMinutes, briefingNotes: briefing.trim() });
    setBusy(false);
  ...
  async function choose(value: OpinionDisposition) {
    if (!room) return;
    setBusy(true); setNotice(null);
    const result = await recordRecoveryDisposition(session.accessToken, room.recoveryCase.id, { disposition: value, note: partyNote.trim() || undefined });
    setBusy(false);
    ...
    {!loading && !error && <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      <div className="space-y-5">
        <section className="rounded-2xl border border-ink/8 bg-white p-5 shadow-sm">
          ...
          {reviews.length === 0 ? <><p className="mt-3 text-sm leading-6 text-ink/55">No participant-visible review case exists. YUI will not manufacture a dispute. Start from the agreement’s real review action when the backend offers it.</p><Link to={`/agreements/${agreementId}`} className="mt-4 inline-flex min-h-11 items-center rounded-full border border-green-700/20 px-4 text-sm font-semibold text-green-700">Back to agreement</Link></> : <>
