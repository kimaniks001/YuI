  ...
  return <TraderShell>
    <div className="mx-auto max-w-4xl space-y-6 py-4 sm:py-8">
      <section className="rounded-3xl border border-green-700/12 bg-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-ink/60">The Market is real. Trainer teaches safely. Game simulates trade and life through consequences. The experience can feel familiar across all three, but their authority must never blur.</p>
      </section>
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <World icon={<BookOpenCheck size={19} />} title="Trainer" label="SIMULATED" body="Guided learning. It can demonstrate SecurePay journeys, but live SecurePayAPI authority is blocked in this world." to="/trainer" action="Open Trainer" />
        <World icon={<Gamepad2 size={19} />} title="Game" label="SIMULATED" body="Practice economy, scenarios and competition. Game resources, rankings and Game Master status cannot become Real Market truth." to="/game" action="Open Game" />
      </div>
      <section className="rounded-2xl border border-ink/8 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <Promise title="Clients request. Backend decides." body="YUI can explain, guide and request actions. It cannot declare agreement authority, Payment Ready, release, settlement, consultation billing, Recovery outcome or real Master status." />
          <Promise title="Simulation stays simulation." body="Trainer and Game carry only safe draft intent toward Market. Real identity, parties, payer, terms and financial authority are established again after authentication." />
          ...
          <Promise title="Recovery is a process, not a verdict." body="Recovery composes the existing Agreement Review record with optional Master advice and party choices. SecurePay records the process without pretending to be a court." />
          <Promise title="Builder rewards stay backend-qualified." body="Referral provenance and ten-month retention entitlement are backend truth. YUI never calls a reward paid until a real payout/posting authority says it is paid." />
        </div>
      </section>
...
function Promise({ title, body }: { title: string; body: string }) { return <div className="rounded-xl border border-ink/8 bg-[#fbfcf9] p-4"><strong className="text-sm text-ink">{title}</strong><p className="mt-1 text-xs leading-5 text-ink/50">{body}</p></div>; }
